...
class WcTest {
    ...
    fun `Should count one file`() {
        ...
        TestUtils.runExecutorTest(wc, "some input", "3 4 27 ${TestUtils.resourcesDir}AnotherFile.txt\n", "")
    }

    @Test
    fun `Should read input if no arguments passed`() {
        val wc = Wc(listOf())
        TestUtils.runExecutorTest(wc, "some input", "0 2 10\n", "")
    }
    ...
    fun `Should calculate total on two files`() {
        ...
        TestUtils.runExecutorTest(wc, "some input",
    ...
    fun `Should print error if cannot find file`() {
        val wc = Wc(listOf("a"))
        TestUtils.runExecutorTest(wc, "some input", "", "some error")
    }
}
